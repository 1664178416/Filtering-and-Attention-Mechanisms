...
\author{\IEEEauthorblockN{1\textsuperscript{st}* Yonghui Wang}
\IEEEauthorblockA{\textit{Computer Science and Engineering} \\
\textit{UESTC}\\
Chengdu, China \\
wangyonghui166@gmail.com}
\and
\IEEEauthorblockN{2\textsuperscript{nd}* Jinyi Liu}
\IEEEauthorblockA{\textit{School of Transportation} \\
\textit{Jilin University}\\
Changchun, China \\
liujy1722@mails.jlu.edu.cn}
\and
\IEEEauthorblockN{3\textsuperscript{rd}† Jiarui Tang}
\IEEEauthorblockA{\textit{Computer Science and Engineering} \\
\textit{UESTC}\\
Chengdu, China \\
tjr030301@163.com}
...
Spiking Neural Networks(SNN),which are regarded as the third generation of neural network models,offer a more biologically plausible simulation of neural dynamic by incorporating not only neuronal and synaptic states but also temporal information into their computational framework\cite{b3,10.5555}.Unlike traditional ANNs that use continuous activations,SNNs process information through discrete spike events,enabling them to capture temporal dependencies and perform energy-efficient computation. This makes SNNs a promising alternative for complex cognitive tasks, offering superior performance and lower power consumption\cite{roy2019towards}.
...
Attention mechanism has attracted considerable attention since its inception and has been widely used in various neural network architectures.Yu et al. (2025) proposed a Frequency-based Spatial-Temporal Attention(FSTA) model to enhance feature learning in SNNs\cite{yu2025}. Xue et al. (2024) introduced Gated Attention Coding (GAC)\cite{qiu2024}, a plug-and-play module that encodes inputs into powerful representations. Shen et al.(2024) innovatively implemented an attention mechanism in SNNs called Temporal Interaction Module (TIM),which was designed to augment the temporal data processing abilities within SNN architectures\cite{shen2024}. Cai et al. (2024) proposed a novel attention mechanism called spatial-channel–temporal-fused attention(SCTFA), which is designed to guide SNNs to efficiently capture underlying target regions by utilizing accumulated historical spatial–channel information in the present study\cite{10138927}. 


\section{Methodology}
in this section, we first introduce the fundamental computational units of SNNs due to its simplicity and biological plausibility\cite{TAVANAEI201947},namely the most widely used spiking neuron:the Leaky Integrate-and-Fire (LIF) neuron\cite{10.5555/2728336.2728412}.Then,we discussed the polarity coherence filtering method used for dataset denoising. At last, we present the model improved with Squeeze-and-Excitation Block (SEBlock)\cite{hu2019}.
...
After completing the dataset filtering process, we used the processed dataset to train the model. In this study, we proposed a model based on WeiFang's model\cite{fang2021}, enhancing it with the Squeeze-and-Excitation Block (SEBlock)\cite{hu2019}.We conducted various experiments on this model and performed detailed comparisons. The model architecture is illustrated in Fig \ref{fig:model}, and more specific details will be presented in the following sections.
\begin{figure}[htbp]
    \centering
...
\textbf{Dataset Description.} In this experiment, we utilize the DVS Gesture dataset, a publicly available dataset for gesture recognition based on Dynamic Vision Sensor (DVS)\cite{9138762}. 
...
The results indicate that models with more frames achieve higher accuracy. As shown in Fig\ref{fig:main_figure}, the frame partitioning strategy has a negligible impact on both model loss and accuracy during the initial 10 training epochs. However, subsequent training reveals a significant performance divergence: higher frame numbers consistently yield superior results. The baseline model achieves lower loss values and higher accuracy when processing frames sampled from certain movements\cite{10.1145/3065386}. This phenomenon can be attributed to the increased information density provided by finer temporal segmentation, enabling more comprehensive feature extraction across the network's deeper layers\cite{726791}.

% The results show that the model with more frames can achieve higher accuracy.The Fig\ref{fig:main_figure} demonstrate that the frame partitioning strategy exhibits negligible impact on both model loss and accuracy during the initial 10 training epochs. However, subsequent training reveals a significant performance divergence-higher frame numbers consistently yield superior results, with the baseline model achieving lower loss values and higher accuracy when processing certain movement sampled frames\cite{10.1145/3065386}. This phenomenon can be attributed to the increased information density afforded by finer temporal segmentation, which enables more comprehensive feature extraction throughout the network's deeper layers. LeCun et al. also demonstrated in their seminal work on gradient-based learning applied to document recognition how different input representations can significantly affect model performance\cite{726791}.
...
In conclusion, this study highlights the importance of data preprocessing and task-aware design in model optimization, providing a foundation for advancing gesture recognition and related applications. Balancing model complexity, efficiency, and accuracy remains key. Task-specific attention and quantization techniques \cite{2021Towards} offer promising paths for optimizing SNNs on resource-constrained devices.
